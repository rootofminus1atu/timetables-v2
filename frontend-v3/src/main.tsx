    ...
    <Route path="/" element={<Root />} loader={rootLoader}>
      ...
      <Route 
        ...
        shouldRevalidate={({ currentUrl, nextUrl }: ShouldRevalidateFunctionArgs) => {
          ...
          const currentId = currentUrl.pathname.split('/')[2]
          const nextId = nextUrl.pathname.split('/')[2]
          console.log(currentId, "vs", nextId)

          return currentId !== nextId || getThisWeeksMonday(new Date(currentDate)).getTime() !== getThisWeeksMonday(new Date(nextDate)).getTime()
        }}
      />
    </Route>
  )
