...
const Appointment3 = ({
  ...
  >
    <div>
      <div>{children}</div>
      <div style={{ fontSize: 12 }}>{data.location}</div>
    </div>
...
const Appointment2 = ({
  ...
  data,
  ...restProps
}: Appointments.AppointmentProps & { style?: React.CSSProperties; data: any; }) => (
    <Appointments.Appointment {...restProps} style={{ ...style, backgroundColor: '#FFC107', borderRadius: '8px' }}>
      <div>
        <div>{children}</div>
        <div style={{ fontSize: 12 }}>{data.location}</div>
      </div>
...
const Lesson = ({ children , data }: Appointments.AppointmentProps & { data: MyData }) => (
    <div>hi</div>
)
...
export default () => (
  <Paper>
    ...
    >
      ...
      />
      <Appointments
        // appointmentContentComponent={Lesson}
      />
    </Scheduler>
  </Paper>
);
