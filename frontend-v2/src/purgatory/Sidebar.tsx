...
const performSearch = async (modules: Module[], q: string) => {
  return modules.filter(t => t.id.toLowerCase().includes(q.toLowerCase()))
}


const Sidebar = ({ modules }: { modules: Module[] }) => {
  const [timetableNames, setTimetableNames] = useState(modules)
  ...
  const handleSearch = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSearching(true)
    const found = await performSearch(modules, query)
    setSearching(false)
        ...
        {timetableNames.length ? (
          <ul className='flex flex-col'>
            {timetableNames.map(t => (
              <NavLink 
                to={`timetable/${encodeURIComponent(t.id)}`}
                key={t.id} 
